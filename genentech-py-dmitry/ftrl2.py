...
def data(path, D):
    ...
    for t, row in enumerate(DictReader(open(path))):
        ...
        yield t, patient_id, xindices, xvalues, y
...
for e in xrange(epoch):
    ...
    xindices = []
    xvalues = [1.0]
    for t, patient_id, xindices, xvalues, y in data(train, D):  # data is a generator
    #    t: just a instance counter
    #    patient_id: patient_id provided in original data
    #    x: features
    #    y: label (click)
        if (patient_id != patient_id_prev) and (t>0):
...
with open(submission, 'w') as outfile:
    ...
    xindices = []
    xvalues = [1.0]
    for t, patient_id, xindices, xvalues, y in data(test, D):
        if (patient_id != patient_id_prev) and (t>0):
            p_prev = learner.predict(xindices_prev, xvalues_prev)
