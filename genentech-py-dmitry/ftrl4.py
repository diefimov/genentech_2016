...
class ftrl_proximal(object):
    ...
    def predict(self, xindices, xvalues):
        ''' Get probability estimation on x

            INPUT:
                xindices: a list of feature indices
                xvalues: a list of feature values
        ...
        wTx = 0.
        j = 0
        for i in self._indices(xindices):
    ...
    def update(self, xindices, p, y):
        ''' Update model using xindices, p, y
...
def data(path, D):
    ...
    for t, row in enumerate(DictReader(open(path))):
        ...
        # process target
        y = 0.
        if 'is_screener' in row:
            if row['is_screener'] == '1':
                y = 1.
            del row['is_screener']
        ...
        yield t, patient_id, claim_id, xindices, xvalues, y
...
for e in xrange(epoch):
    ...
    xindices = []
    xvalues = [1.0]
    for t, patient_id, claim_id, xindices, xvalues, y in data(train, D):  # data is a generator
    #    t: just a instance counter
    #    patient_id: patient_id provided in original data
    #    x: features
    #    y: label (click)
        if (claim_id != claim_id_prev) and (t>0):
            # step 1, get prediction from learner
            p_prev = learner.predict(xindices_prev, xvalues_prev)
            # step 2, update weights
            learner.update(xindices_prev, p_prev, y_prev)
        
        claim_id_prev = claim_id
        xindices_prev = xindices
        xvalues_prev = xvalues
        y_prev = y

        #print progress
        localcount += 1
        if localcount % 10000000 == 0:
            print "train: " + str(localcount)

    # prediction for the last patient
    p_prev = learner.predict(xindices_prev, xvalues_prev)
    learner.update(xindices_prev, p_prev, y_prev)
...
with open(submission, 'w') as outfile:
    outfile.write('patient_id,is_screener_pred\n')
    claim_id_prev = 0
    claim_id = 0
    xindices = []
    xvalues = [1.0]
    for t, patient_id, claim_id, xindices, xvalues, y in data(test, D):
        if (claim_id != claim_id_prev) and (t>0):
            p_prev = learner.predict(xindices_prev, xvalues_prev)
            outfile.write('%s, %f\n' % (str(patient_id_prev), p_prev))
