...
for e in xrange(epoch):
    ...
    patient_id_prev = 0
    xindices_prev = []
    for t, patient_id, xindices, y in data(train, D):  # data is a generator
    ...
    #    y: label (click)
        if patient_id != patient_id_prev:
            ...
            p = learner.predict(xindices)
            # step 2, update weights
            learner.update(xindices, p, y)
        else:
            p = learner.predict(list(set(xindices + xindices_prev)))
            learner.update(list(set(xindices + xindices_prev)), p, y)
...
with open(submission, 'w') as outfile:
    ...
    patient_id_prev = 0
    xindices_prev = []
    for t, patient_id, xindices, y in data(test, D):
        if patient_id != patient_id_prev:
            p = learner.predict(xindices)
            outfile.write('%s, %f\n' % (str(patient_id), p))
        else:
